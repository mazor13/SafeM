...
echo "🚀 Implementing Smart Account Lookup & Demo Data..."
...
import { 
  ...
  Layout, Trash2, Edit2, Save, CheckCircle2, Search, Database 
} from 'lucide-react';
import { 
...
export default function OpportunitiesPage() {
  ...
  const handleAccountSearch = async (text: string) => {
    ...
    // אם המשתמש מוחק את הטקסט, נקה ID
    if (!text) {
    ...
    // Debounce search (חכה שהמשתמש יפסיק להקליד)
    if (searchTimeout.current) clearTimeout(searchTimeout.current);

    searchTimeout.current = setTimeout(async () => {
        try {
            // חיפוש פשוט ב-Firestore (Simulation: in real app use Algolia or specialized index)
            // כאן נביא את כל ה-Tenants ונסנן בקוד (לא יעיל ל-Scale אבל מצוין ל-Staging)
  ...
  const createDemoTenants = async () => {
      const demos = ['Google', 'Microsoft', 'NVIDIA', 'Osem', 'Strauss Group', 'Elbit Systems'];
      ...
      alert('נוצרו חברות דמו! נסה לחפש אותן כעת.');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.accountId) {
        if(!confirm('לא נבחר לקוח קיים מהרשימה. האם ליצור הזדמנות ללא קישור ללקוח? (לא מומלץ)')) return;
    }
    ...
    <div className="flex flex-col h-screen p-6 overflow-hidden" dir="rtl">
      ...
      <div className="flex justify-between items-start mb-6 shrink-0">
        ...
        <div className="flex gap-3">
             <button onClick={createDemoTenants} className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-xl font-bold text-sm flex gap-2 items-center border border-white/5">
              <Database size={16} /> טען נתוני דמו
            </button>
            <button 
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="relative">
                      ...
                      <div className="relative">
                        ...
                        {formData.accountId && <CheckCircle2 size={16} className="absolute top-3.5 right-3 text-emerald-500" />}
                      </div>
                      ...
                      {showDropdown && searchResults.length > 0 && (
                          <div className="absolute z-50 w-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                              {searchResults.map(res => (
                                  <div 
                                    key={res.id} 
                                    onClick={() => selectAccount(res)}
                                    className="p-3 hover:bg-indigo-600/20 cursor-pointer border-b border-white/5 last:border-0 flex items-center justify-between"
                                  >
                                      <span className="font-bold text-white">{res.name}</span>
                                      <span className="text-xs text-slate-400">ID: {res.id.substr(0,4)}</span>
                                  </div>
                              ))}
                          </div>
                      )}
                      
                      {showDropdown && searchResults.length === 0 && formData.accountName && (
                          <div className="absolute z-50 w-full mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-3 text-center text-slate-400 text-sm">
                              לא נמצאו תוצאות. 
                              <br/>
                              <span className="text-xs">(לחץ "טען נתוני דמו" במסך הראשי אם הרשימה ריקה)</span>
                          </div>
                      )}
                    </div>
...
echo "✅ Smart Search & Demo Data button added. Deploying..."
